...
pub trait TaxRepository: Send + Sync {
    ...
    async fn find_effective_by_company(
        &self,
        company_id: Uuid,
        date: &time::Date,
    ) -> Result<Vec<TaxType>, AppError>;
...
pub trait ReportRepository: Send + Sync {
    async fn cash_position(
        ...
        company_id: Uuid,
        as_of: time::Date,
    ) -> Result<crate::dto::report::CashPositionReport, AppError>;
    async fn profit_loss(
        &self,
        company_id: Uuid,
        period_from: time::Date,
        period_to: time::Date,
    ) -> Result<crate::dto::report::ProfitLossReport, AppError>;
}
