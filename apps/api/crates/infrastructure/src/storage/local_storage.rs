...
impl StoragePort for LocalStorageProvider {
    async fn upload(&self, filename: &str, data: Vec<u8>, _content_type: &str) -> Result<String, AppError> {
    ...
    async fn download(&self, path: &str) -> Result<Vec<u8>, AppError> {
        ...
        fs::read(full_path).await.map_err(|e| AppError::Internal(e.into()))
    }

    async fn delete(&self, path: &str) -> Result<(), AppError> {
        let full_path = self.base_path.join(path);
        fs::remove_file(full_path).await.map_err(|e| AppError::Internal(e.into()))
    }
